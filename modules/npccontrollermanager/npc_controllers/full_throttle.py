...
class FullThrottleControllerProcess:
    ...
    def do(self):
        ...
        if self.rear_axle_position.any() and dt:
            ...
            if (time_stamp - self.initial_time) * 1e-9 < self.settings.full_throttle_duration:
                throttle = 1.
            # elif (time_stamp - self.initial_time) * 1e-9 < (1.3 * self.settings.full_throttle_duration):
            #     throttle = 0.
            else:
                throttle = -1.
...
class FullThrottleSettings:
    def __init__(self):
        self.controller_type = NPCControllerTypes.FULL_THROTTLE

        self.kp = 150
        self.kd = 10

        self.full_throttle_duration = 10.
    ...
    def __str__(self):
        return str('Full Throttle Controller Settings')
...
class FullThrottleSettingsDialog(QtWidgets.QDialog):
    def __init__(self, module_manager, settings: FullThrottleSettings, parent=None):
        super().__init__(parent=parent)
        self.module_manager = module_manager
        self.full_throttle_settings = settings
        uic.loadUi(os.path.join(os.path.dirname(os.path.realpath(__file__)), "ui/full_throttle_settings_ui.ui"), self)
    ...
    def accept(self):
        self.full_throttle_settings.reference_trajectory_name = self.trajectoryComboBox.currentText()
        self.full_throttle_settings.static_look_ahead_distance = self.staticLADDoubleSpinBox.value()
        self.full_throttle_settings.use_dynamic_look_ahead_distance = self.dynamicLADCheckBox.isChecked()
        self.full_throttle_settings.steering_gain = self.steeringGainDoubleSpinBox.value()
        self.full_throttle_settings.dynamic_lad_a = self.aDoubleSpinBox.value()
        self.full_throttle_settings.dynamic_lad_b = self.bDoubleSpinBox.value()
        self.full_throttle_settings.kp = self.kpDoubleSpinBox.value()
        self.full_throttle_settings.kd = self.kdDoubleSpinBox.value()
        self.full_throttle_settings.full_throttle_duration = self.durationDoubleSpinBox.value()

        super().accept()

    def display_values(self, settings=None):
        if not settings:
            settings = self.full_throttle_settings
        ...
        self.durationDoubleSpinBox.setValue(settings.full_throttle_duration)
