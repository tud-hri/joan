...
class FDCAControllerDuecaSettingsDialog(QtWidgets.QDialog):
    def __init__(self, settings, module_manager, parent=None):
        ...
        self.fdca_controller_dueca_settings = settings
        self.module_manager = module_manager

        uic.loadUi(os.path.join(os.path.dirname(os.path.realpath(__file__)), "ui/fdcadueca_settings_ui.ui"), self)
        self._path_trajectory_directory = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'trajectories')

        self.btnbox_fdca_controller_dueca_settings.button(
            self.btnbox_fdca_controller_dueca_settings.RestoreDefaults).clicked.connect(self._set_default_values)
    ...
    def update_parameters(self):
        ...
        self.fdca_controller_dueca_settings.k_y = float(self.edit_k_y.text())
        self.fdca_controller_dueca_settings.k_psi = float(self.edit_k_psi.text())
        self.fdca_controller_dueca_settings.lohs = float(self.edit_lohs.text())
        self.fdca_controller_dueca_settings.sohf = float(self.edit_sohf.text())
        self.fdca_controller_dueca_settings.loha = float(self.slider_loha.value())
        self.fdca_controller_dueca_settings.trajectory_name = self.cmbbox_hcr_selection.itemText(
    ...
    def _display_values(self, settings_to_display=None):
        if not settings_to_display:
            settings_to_display = self.fdca_controller_dueca_settings
    ...
    def update_trajectory_list(self):
        ...
        idx = self.cmbbox_hcr_selection.findText(self.fdca_controller_dueca_settings.trajectory_name)
        if idx != -1:
            self.cmbbox_hcr_selection.setCurrentIndex(idx)

class FDCAControllerDuecaProcess:
...
class FDCAControllerDuecaSettings:
    def __init__(self, identifier=''):
        ...
        self.haptic_controller_type = HapticControllerTypes.FDCA_DUECA.value
