...
class HapticControllerManagerProcess(ModuleProcess):
    ...
    def get_ready(self):
        for key, value in self._settings_as_object.haptic_controllers.items():
            self.haptic_controller_objects[key] = HapticControllerTypes(value.haptic_controller_type).process(settings=value, shared_variables=self._module_shared_variables.haptic_controllers[key], carla_interface_settings = self.settings_carla_interface )
        """
