...
class HardwareMPProcess(ModuleProcess):
    ...
    def get_ready(self):
        ...
        for joysticks in self._sharedvalues_module.joysticks:
            self.input_classes[joysticks] = HardwareInputTypes.JOYSTICK.klass_mp(settings = self.settings['Hardware MP']['joy_sticks'][j], shared_values = self._sharedvalues_module.joysticks[joysticks])
            j += 1
