...
class TemplateWidget(Control):
    def __init__(self, *args, **kwargs):
        ...
        self.createWidget(ui=os.path.join(os.path.dirname(
            os.path.realpath(__file__)), "template.ui"))
        ...
        self.defineModuleStateHandler(
            module=self, moduleStates=TemplateStates())
    ...
    def start(self):
        ...
        self.moduleStateHandler.requestStateChange(
            self.moduleStates.TEMPLATE.RUNNING)
        self.startPulsar()

    def stop(self):
        self.moduleStateHandler.requestStateChange(
            self.moduleStates.TEMPLATE.STOPPED)
        self.stopPulsar()
    ...
    def handlemasterstate(self, state):
        ...
        try:
            ...
            # ensure we have the State object (not the int)
            stateAsState = self.masterStateHandler.getState(state)
    ...
    def handlemodulestate(self, state):
        ...
        try:
            # stateAsState = self.states.getState(state) # ensure we have the State object (not the int)
            # ensure we have the State object (not the int)
            stateAsState = self.moduleStateHandler.getState(state)
            ...
            if stateAsState == self.moduleStates.TEMPLATE.RUNNING:
                self.stateWidget.btnStart.setStyleSheet(
